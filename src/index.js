...
function add_row_to_cart(id) {
  ...
  $('.popup #item' + id + ' .cart_minus_block').on('click', function (event) {
    products[id]--;
    recalculate_row($('#item' + id+'.cart_row'), id);
    if (products[id] == 0) cart_remove( id);
  });
  $('.popup #item' + id + ' .cart_plus_block').on('click', function (event) {
    products[id]++;
    recalculate_row($('#item' + id+'.cart_row'), id);
  });
  $('.popup #item' + id + ' .row_close').on('click', function (event) {
    products[id]=0;
    recalculate_row($('#item' + id+'.cart_row'), id);
    cart_remove(id);
  });
...
function cart_add(id) {
  ...
  if (products[id] == 0) add_row_to_cart(id);
    ...
    $('.popup #item' + id + ' .cart_amount_number')[0].innerText=products[id]+1;
  }
  products[id]++;
  recalculate_row($('#item' + id+'.cart_row'), id);
}
